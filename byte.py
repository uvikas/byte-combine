...
model = Model(EMBED_DIM, DIM1, DIM2, DIM3)
...
def train(epochs):
    ...
    for epoch in range(epochs):
        ...
        for mini in range(MINI_BATCH):
            optimizer.zero_grad()
            x_train, t_train = datagen(BATCH_SIZE)
        ...
        if(epoch % 1 == 0):
            print('train %d: %.5f' %(epoch, losses[len(losses)-1]))
    return losses
